...
// Single source of truth for ALL corridor pages (I-5 included). Each page: live
// multi-state camera+alert map filtered to that route, a right-side detail card
// (click a marker), a smart critical-closure banner, pass/high-point POI markers,
...
const CORRIDORS = [
  {
    ...
    pois: [ {name:'Siskiyou Summit', slug:'siskiyou', lat:42.0672, lon:-122.5606},
            {name:'The Grapevine', slug:'grapevine', lat:34.7947, lon:-118.8790} ],
    hero: `See I-5 right now — live DOT traffic cameras and real-time road conditions from Blaine, WA to the Mexican border, each tagged with route and mile marker. Check Siskiyou Summit, the Grapevine, and every pass before you drive.`,
    segs: [
      ['I-5 in Washington', `From the Columbia River at Vancouver north through Kelso, Olympia, Joint Base Lewis-McChord (a chronic congestion choke point), Tacoma, Seattle, and Everett up to the Peace Arch crossing at Blaine. Snow is rare at sea level, but the Seattle–Tacoma stretch is one of the most congested corridors in the country — the cameras here are mostly about traffic, not weather.`],
      ['I-5 in Oregon', `Portland, Salem, Albany, Eugene, Roseburg, Grants Pass, Medford, and Ashland — then the climb to <a href="../../passes/siskiyou/">Siskiyou Summit</a> at 4,310 feet, the highest point on the entire interstate. This is the winter wildcard: chains, spinouts, and full closures are common from November through March.`],
      ['I-5 in California', `Down from the Siskiyous past Mount Shasta and Redding, through Sacramento and the length of the Central Valley, over <a href="../../passes/grapevine/">the Grapevine (Tejon Pass, 4,144 ft)</a> into Los Angeles, and on to San Diego. The Grapevine is the other notorious closure point — snow and high wind shut it down several times each winter.`],
    ],
    faq: [
  ...
  },
  {
    ...
    pois: [ {name:'Snoqualmie Pass', slug:'snoqualmie', lat:47.3923, lon:-121.4001} ],
    ...
    faq: [
      ...
      ['What mountain passes does I-90 cross?', `The big one is <a href="../../passes/snoqualmie/">Snoqualmie Pass</a> in the Washington Cascades (marked ▲ on the map). See all Western <a href="../../passes/">mountain pass conditions</a> for the ones on your route.`],
      ['Where does I-90 get the most snow?', `Snoqualmie Pass, the northern plains of Montana and South Dakota, and the Great Lakes snow belt. Track your mile marker through any of it with the <a href="https://apps.apple.com/us/app/milecheck/id6759212851" target="_blank" rel="noopener">MileCheck app</a>.`],
    ],
  },
  {
    ...
    faq: [
      ...
      ['Does I-95 close for hurricanes or snow?', `Yes — Atlantic hurricanes and flooding hit the Southeast section, and winter storms ice the Northeast. Watch the storms behind them on the <a href="../../weather/">road weather map</a>.`],
    ],
  },
  {
    ...
    pois: [ {name:'Donner Pass', slug:'donner', lat:39.3126, lon:-120.3269},
            {name:'Parleys Summit', slug:'parleys', lat:40.7519, lon:-111.6377} ],
...
function page(c){
  ...
  const hpStat = c.highPass
    ? `    <a class="co-stat co-stat-link" href="../../passes/${c.highPass.slug}/"><div class="n" style="font-size:17px;padding-top:3px">${c.highPass.name}</div><div class="l">${c.highPass.elev} · high point ▸</div></a>`
    : `    <div class="co-stat"><div class="n" style="font-size:16px;padding-top:4px">${c.subtitle.split('·')[0].trim()}</div><div class="l">${c.lengthMi} end to end</div></div>`;
  return `<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    a.co-stat-link{text-decoration:none;color:inherit;transition:border-color .15s;}
    a.co-stat-link:hover{border-color:#0f7a4f;}
    a.co-stat-link .l{color:#0f7a4f;}
    ...
    .co-card .cc-link{display:inline-block;margin-top:9px;font-weight:700;color:#0f7a4f;text-decoration:none;font-size:13.5px;}
    .poi-icon{color:#7C3AED;font-size:22px;line-height:1;text-align:center;text-shadow:0 0 3px #fff,0 0 4px #fff,0 0 5px #fff;cursor:pointer;font-weight:900;}
    ...
    .co-seg a{color:#0f7a4f;font-weight:700;text-decoration:none;}
...
async function alertsFor(st){try{const d=await fetch(WORKER+'/incidents?state='+st).then(r=>r.json());return (d['incident-reports']||[]).filter(a=>{const rid=a.location&&a.location['route-id'];const sl=a.location&&a.location['start-location'];return isRoute(rid)&&sl&&+sl['start-lat'];}).map(a=>{const sl=a.location['start-location'];return {lat:+sl['start-lat'],lon:+sl['start-long'],type:a['event-type-id']||'OT',mp:sl['start-mile-marker'],title:(a.headline||a.description||a['impact-desc']||'Incident').replace(/<[^>]+>/g,' ').replace(/\\s+/g,' ').trim().slice(0,140),desc:(a.description||a['impact-desc']||'').replace(/<[^>]+>/g,' ').replace(/\\s+/g,' ').trim().slice(0,240),st};});}catch(e){return [];}}
...
POIS.forEach(p=>L.marker([p.lat,p.lon],{icon:L.divIcon({className:'',html:'<div class="poi-icon" title="'+esc(p.name)+'">▲</div>',iconSize:[26,26],iconAnchor:[13,13]}),zIndexOffset:500}).on('click',()=>showCard(poiCard(p),false)).addTo(poiLayer));
...
Promise.all([Promise.all(STATES.map(camsFor)),Promise.all(STATES.map(alertsFor))]).then(([cams,alerts])=>{
  ...
  document.getElementById('statCams').textContent=CAMS.length;document.getElementById('statAlerts').textContent=ALERTS.length;
  draw();
  const fc=ALERTS.filter(isFullClosure);
  if(fc.length){const f=fc[0];const loc=NAME+(f.mp>0?' · MP '+Math.round(f.mp):'')+' · '+f.st;const bn=document.getElementById('critBanner');bn.innerHTML='⚠ CRITICAL: '+NAME+' has '+fc.length+' active full-closure alert'+(fc.length>1?'s':'')+' — nearest at '+loc+(fc.length>1?' (and '+(fc.length-1)+' more)':'')+'. Tap a red marker on the map for details.';bn.style.display='block';}
}).catch(()=>{document.getElementById('coStatus').textContent='Live data unavailable right now — try again shortly.';});
</script>
