...
const ROAD_RE=/^(I|US|SR|SH|WA|OR|UT|MT|AZ|AL|NV|WI|NY|LA|GA|SC|CA|SD|FL|MI|VT|NH|ME|PA|NE|KS|IA|MN|IN|DE|CT|QEW|BC|ON|AB|M|Loop|\\d)[- ]?\\d*/i;
...
// HLS playback for stream cameras (DelDOT publishes video only, 2026-09-25): Safari plays the playlist itself, everyone else gets hls.js, loaded once on the first stream card. One player at a time; closing the card stops it.
...
function playStream(url){var v=document.getElementById('ccVid');if(!v)return;if(v.canPlayType('application/vnd.apple.mpegurl')){v.src=url;v.play().catch(function(){});return;}loadHls().then(function(){if(!window.Hls||!Hls.isSupported())return;hlsInst=new Hls({liveSyncDurationCount:3});hlsInst.loadSource(url);hlsInst.attachMedia(v);hlsInst.on(Hls.Events.MANIFEST_PARSED,function(){v.play().catch(function(){});});}).catch(function(){});}
