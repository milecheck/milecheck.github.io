...
const STATES = [
  ...
  { slug:'nebraska', code:'NE', name:'Nebraska', dot:'NDOT', bounds:'[[40,-104.05],[43,-95.31]]',
   ...
   { slug:'kansas', code:'KS', name:'Kansas', dot:'KDOT', bounds:'[[37.00,-102.05],[40.00,-94.59]]',
     blurb:`KDOT's cameras follow I-70 across the state and I-35 and I-135 through Wichita and up to Salina, and the Kansas City Scout network covers the metro on both sides of the state line, I-435, I-635 and US-69.`,
   ...
   { slug:'delaware', code:'DE', name:'Delaware', dot:'DelDOT', bounds:'[[38.45,-75.79],[39.84,-75.04]]',
     blurb:`DelDOT's cameras are live video, one stream per camera, on I-95 and I-495 through Wilmington, US-13 the length of the state and DE-1 down to the beaches. Tap a camera and it plays in place. Cameras on roads DelDOT names rather than numbers are not on this map.`,
     notable:`Watch <a href="../../corridors/i-95/">I-95</a> through Wilmington, DE-1 from Dover to Rehoboth on a summer Friday, and US-13 and US-113 down the middle of the state. DelDOT publishes where its plows are only while a snow event is on.` },
   { slug:'west-virginia', code:'WV', name:'West Virginia', dot:'WVDOT (WV511)', bounds:'[[37.20,-82.65],[40.64,-77.72]]',
     blurb:`West Virginia's cameras are live video from WV511, on the interstates that cross the mountains, I-64, I-77 and I-79, and on the US and state routes around Charleston, Morgantown and the Eastern Panhandle. Tap a camera and it plays in place. Some streams go offline for stretches, and the card says so when one does. Cameras on roads WVDOT names rather than numbers are not on this map.`,
     notable:`Watch I-64 and I-77 through Charleston and over the West Virginia Turnpike, I-79 from Morgantown south to Clarksburg, I-81 through Martinsburg in the Eastern Panhandle, and US-19 at the New River Gorge Bridge.` },
   ...
   { slug:'illinois', code:'IL', name:'Illinois', dot:'IDOT (Getting Around Illinois)', bounds:'[[36.97,-91.52],[42.51,-87.02]]',
     blurb:`IDOT's cameras are thickest in the Chicago area, on the Kennedy, Dan Ryan, Eisenhower and Edens and the I-294 and I-355 tollways, with more on I-55, I-57, I-74 and I-80 downstate. Cameras on roads IDOT names rather than numbers are not on this map.`,
     notable:`Watch <a href="../../corridors/i-90/">I-90</a> and <a href="../../corridors/i-94/">I-94</a> through Chicago, <a href="../../corridors/i-80/">I-80</a> across the south suburbs to the Quad Cities, I-55 toward St. Louis, and I-74 over the Illinois River in Peoria.` },
   ...
   { slug:'north-carolina', code:'NC', name:'North Carolina', dot:'NCDOT (DriveNC)', bounds:'[[33.84,-84.32],[36.59,-75.46]]',
     blurb:`NCDOT's cameras on DriveNC run along I-40 from the mountains to Wilmington, I-85 and I-77 through Charlotte, I-95 down the east, I-26 and I-40 around Asheville, and the Raleigh and Charlotte beltways, with more on US and NC routes toward the coast. Most carry a mile marker.`,
     notable:`Watch I-40 through the Pigeon River Gorge and across the state, I-77 and I-85 through Charlotte, <a href="../../corridors/i-95/">I-95</a> down the east, and US-64 and NC-12 toward the Outer Banks.` },
  ...
  { slug:'new-mexico', code:'NM', name:'New Mexico', dot:'NMDOT (NMRoads)', bounds:'[[31.33,-109.05],[37,-103]]',
    blurb:`NMDOT's NMRoads cameras cover the Albuquerque and Santa Fe metros and the long interstates across the state.`,
    notable:`Watch I-25 up La Bajada between Albuquerque and Santa Fe, I-40 across the Continental Divide and Santa Rosa, and <a href="../../corridors/i-10/">I-10</a> through Las Cruces.` },
  { slug:'texas', code:'TX', name:'Texas', dot:'TxDOT', bounds:'[[25.84,-106.65],[36.5,-93.51]]',
    blurb:`TxDOT's cameras cover Houston, Dallas-Fort Worth, San Antonio, Austin and El Paso, plus the interstates between them.`,
    notable:`Watch I-35 through Austin and San Antonio, <a href="../../corridors/i-10/">I-10</a> from El Paso to Houston, and I-45 between Dallas and Houston.` },
  { slug:'missouri', code:'MO', name:'Missouri', dot:'MoDOT', bounds:'[[35.99,-95.77],[40.61,-89.1]]',
    blurb:`MoDOT's cameras are mostly live video, concentrated around St. Louis and Kansas City, with more along the interstates between them.`,
...
${ES_PAGES.has(s.slug) ? `  <link rel="alternate" hreflang="en" href="https://milecheckapp.com/cameras/${s.slug}/">\n  <link rel="alternate" hreflang="es" href="https://milecheckapp.com/es/cameras/${s.slug}/">\n${PT_PAGES.has(s.slug) ? `  <link rel="alternate" hreflang="pt" href="https://milecheckapp.com/pt/cameras/${s.slug}/">\n` : ''}  <link rel="alternate" hreflang="x-default" href="https://milecheckapp.com/cameras/${s.slug}/">` : ''}
  ...
  <style>
    ...
    .co-card img.cc-img,.co-card video.cc-img{width:100%;border-radius:8px;margin-top:8px;display:block;background:#f0f0ee;min-height:40px;}
...
function playStream(url){var v=document.getElementById('ccVid');if(!v)return;var native=function(){v.addEventListener('error',streamOffline,{once:true});v.src=url;var go=function(){v.play().catch(function(){});};v.addEventListener('loadedmetadata',go,{once:true});go();};loadHls().then(function(){if(window.Hls&&Hls.isSupported()){hlsInst=new Hls({liveSyncDurationCount:3});hlsInst.loadSource(url);hlsInst.attachMedia(v);hlsInst.on(Hls.Events.MANIFEST_PARSED,function(){v.play().catch(function(){});});hlsInst.on(Hls.Events.ERROR,function(ev,d){if(d&&d.fatal)streamOffline();});}else{native();}}).catch(native);}
// A stream the DOT has switched off answers with an empty playlist (WV511 sent 204 for most of its cameras on 2026-09-25). Say so instead of leaving a blank player.
...
fetchJSON(WORKER+'/cameras?state='+CODE,4).then(d=>{
  ...
  CAMS=(d.cameras||[]).filter(c=>c.isActive!==false&&isFinite(+c.lat)&&+c.lat&&(c.imageUrl||c.streamUrl)&&(ROAD_RE.test((c.route||'').trim())||FERRY_RE.test(c.route||'')||(CODE==='AK'&&AK_HWY_RE.test(c.route||'')))).map(c=>({lat:+c.lat,lon:+c.lon,title:c.title||'Traffic camera',route:(c.route||'').trim(),mp:c.mile,img:c.imageUrl||'',stream:c.imageUrl?'':(c.streamUrl||'')}));
