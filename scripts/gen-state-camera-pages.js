...
function playStream(url){var v=document.getElementById('ccVid');if(!v)return;var native=function(){v.src=url;var go=function(){v.play().catch(function(){});};v.addEventListener('loadedmetadata',go,{once:true});go();};loadHls().then(function(){if(window.Hls&&Hls.isSupported()){hlsInst=new Hls({liveSyncDurationCount:3});hlsInst.loadSource(url);hlsInst.attachMedia(v);hlsInst.on(Hls.Events.MANIFEST_PARSED,function(){v.play().catch(function(){});});}else{native();}}).catch(native);}
